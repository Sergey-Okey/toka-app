            ...
            <div class="form-group">
              ...
              <div class="tags-input-container">
                <div class="tags-input">
                  ...
                  />
                  <button type="button" class="add-tag-button" @click="addTag" :disabled="!newTag">
                ...
                <div class="tags-list">
                  <span
                    ...
                    :key="tag.name"
                    class="tag"
                    :style="{ backgroundColor: tag.color || '#31a974', color: '#fff' }"
                  >
                    {{ tag.name }}
        ...
        <transition-group name="task-list">
          <div
            ...
            :class="{
              ...
              [String(task.priority)]: task.priority && task.priority !== 'none',
            }"
          >
            ...
            <div class="task-content" @click="openEditor(task)">
              <div class="task-header">
                ...
                <div class="task-priority" v-if="task.priority && task.priority !== 'none'">
                  <span :class="String(task.priority)">{{
...
const openEditor = (task: Task | null = null) => {
  if (task) {
    ...
    form.value = { ...task, tags: task.tags ? [...task.tags] : [] }
  } else {
...
const addTag = () => {
  ...
  if (tagName && !form.value.tags?.some((t: Tag) => t.name === tagName)) {
    form.value.tags?.push({ name: tagName, color: newTagColor.value })
  }
  newTag.value = ''
  newTagColor.value = '#31a974'
}

const removeTag = (tagName: string) => {
  form.value.tags = form.value.tags?.filter((t: Tag) => t.name !== tagName) || []
}
...
.search-input:focus {
  ...
  box-shadow: 0 0 0 3px rgba(var(--primary-rgb), 0.15), inset 0 1px 2px rgba(0, 0, 0, 0.05);
  outline: none;
}
